...
しかも当日朝はJR宇都宮線ダイヤが乱れてたりして、参加者のみなさんが無事につけるか((<心配|URL:http://togetter.com/li/222458>))でした。

今回の会場は東那須野公民館で、ここは((<とちぎRuby会議01|URL:http://jp.rubyist.net/magazine/?0026-TochigiRubyKaigi01Report>))が開催された場所でした。
...
つぎに良いコミットを目指そう、"良い"とは何かを説明されました。
他人に読まれてはずかしくないコミットオブジェクトのつくりかた、コミットコメントの書き方を実例を交えて紹介しました。
"ソーシャルコーティングとしてのプログラミングとは日々のコミットを((*紡ぐ*))作業"だと述べられて、素敵な言いかただなと思いました。

"人のバグを直す"を直すというプラクティスを紹介されましたが、Railsはバグの宝庫なのでオススメ(?)だそうです。
人のプロダクトを使ってみてバグを踏んだら直してpull requestするのはマナー、pull requestするときのプラクティスも紹介されます。
...
松田さんといえばRailsのイメージが強いのですが、とちぎRubyではRailsの濃い話についていける人がすくなさそうな気配を感じられたのか、RubyとGitHubによるソーシャルコーディングをすすめて、みんな恥ずかしがらずにコードを晒そうぜ!という強いメッセージを送っていただけました。
...
ささたつさんは笑顔を増やすミッションを楽しく遂行している様子が印象的な講演でした。
...
勉強しながらわからないことや訊いてみたいことを自由に発言し、知っている人がそれに答えるというやりとりがあり、エンコーディングに関する話題で盛り上がりました。
クックパッドにはコーディングガイドってあるの?という質問には松田さんは「Railsのスタイルは意識する((<URL:http://edgeguides.rubyonrails.org/contributing_to_ruby_on_rails.html#follow-the-coding-conventions>))」と回答されていました。
